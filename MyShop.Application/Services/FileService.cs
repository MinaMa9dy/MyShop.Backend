    ...
    public class FileService : IFileService
    {
        ...
        public async Task<Result<string>> SaveFileAsync(IFormFile file, string folderName)
        {
            if (file == null || file.Length == 0)
                return Result<string>.Failure("The file is empty", ErrorCode.VALIDATION_ERROR);
            ...
            return Result<string>.Ok(trustedFileName, "File saved successfully");
        }
        ...
        public async Task<Result<List<string>>> SaveFilesAsync(List<IFormFile> files, string folderName)
        {
            ...
            return Result<List<string>>.Ok(savedFileNames);
        }

        public Result<bool> DeleteFile(string fileName, string folderName)
        {
            if (string.IsNullOrEmpty(fileName))
                return Result<bool>.Failure("The file name is empty", ErrorCode.VALIDATION_ERROR);

            var filePath = Path.Combine(_webHostEnvironment.WebRootPath, folderName, fileName);

            if (!File.Exists(filePath))
                return Result<bool>.Failure("The file does not exist", ErrorCode.NOT_FOUND);
        ...
        {
            if (string.IsNullOrEmpty(fileName))
                return Result<string>.Failure("The file name is empty", ErrorCode.VALIDATION_ERROR);

            var filePath = Path.Combine(_webHostEnvironment.WebRootPath, folderName, fileName);

            if (!File.Exists(filePath))
                return Result<string>.Failure("The file does not exist", ErrorCode.NOT_FOUND);
